...
class Trie {
    ...
    /**Adds the given to the Trie.@param {string} word Word that is being added to the Trie @returns {boolean} true/false status of adding the word*/
    add(word) {
    ...
    /**Searches for a Complete word. This algo does not check for substrings. @param {string} word Word to search for in the Trie @returns {boolean} true/false was the word found*/
    contains(word) {
    ...
    /**Recursively print the characters in the Trie @param {TrieNode} node  The current node.*/
    printAllKeys() {}

    /**Recursively find the number of characters in the Trie Note: the empty string at the root counts as size = 0 @param {TrieNode} node the current node  @param {int} size the size of the Trie
...
// console.log(trieTest);
console.log("then?", trieTest.contains("then"));
console.log("any?", trieTest.contains("any"));
